...
describe("elemento", () => {
  ...
  describe("createCommentForm", () => {
    ...
    it("deve aplicar classes CSS corretas", () => {
      ...
      expect(elements.textarea.className).toContain(
        CSS_CLASSES.COMMENT_TEXTAREA
      );
    });
    ...
    it("deve definir atributos ARIA para acessibilidade", () => {
      ...
      expect(elements.form.getAttribute("aria-label")).toBe(
        "Formulário de anotação do chamado"
      );
      expect(elements.textarea.getAttribute("aria-label")).toBe(
        "Campo de anotação do chamado"
      );
    });
    ...
    it("deve aplicar configuração padrão", () => {
      ...
      expect(elements.textarea.getAttribute("placeholder")).toBe(
    ...
    it("deve incluir header com título", () => {
      ...
      const header = elements.form.querySelector(
        `.${CSS_CLASSES.COMMENT_HEADER}`
      );
  ...
  describe("integração - uso no DOM", () => {
    it("deve funcionar quando anexado ao documento", () => {
      ...
      const formInDom = document.querySelector(
        `#${ELEMENT_IDS.COMMENTS_TEXTAREA}`
      );
