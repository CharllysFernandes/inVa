...
// Estruturas de dados para rastreamento de inputs e estados
...
let emailPollTimeout = null;

function normalize(value) {
  return value.trim().toLowerCase();
}

function clearEmailPolling() {
...
function compareUsernameWithUserEmail(username) {
...
async function showUsernameAlert() {
...
function updateUsernameAlert(isMatch) {
  if (isMatch) {
...
function handleInputValueChange(input) {
...
function setupUsernameInput(input) {
  if (observedInputs.has(input)) {
    return;
  }
...
function connectDiscoveryObserver() {
  ...
  const collectUsernameInputs = (node) => {
    ...
    if (node instanceof Element || node instanceof DocumentFragment) {
      inputs.push(
        ...Array.from(
          node.querySelectorAll("#output_customer_username")
        )
      );
    }
    return inputs;
...
function startUserEmailPolling() {
...
// Initialization logic wrapped in an IIFE to avoid top-level code errors
(function initializeMonitor() {
  ...
  const existingInputs = document.querySelectorAll("#output_customer_username");
